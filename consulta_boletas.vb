...
#Region " Código generado por el Diseñador de Windows Forms "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        Dim resources As System.Resources.ResourceManager = New System.Resources.ResourceManager(GetType(consulta_boletas))
        ...
        Me.Label9.Size = New System.Drawing.Size(72, 8)
        ...
        Me.BTNRETORNAR.Location = New System.Drawing.Point(8, 136)
        ...
        Me.Name = "consulta_boletas"
        Me.Text = "Detalle Boletas"
        Me.TopMost = True
...
#End Region
    Private Sub consulta_boletas_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.TXTAPELLIDOS.Text = ""
    ...
    Sub LLENARLISTA()
        Try
            ...
            cn.Open()
            cm.Connection = cn
            cm.CommandText = "select cm.c_codigo, codigo_boleta,convert(varchar(10),c_fecha,103)as fecha,total,pe_nombre+' '+pe_apellido,pv_descripcion from comprobantes c inner join boletas_de_ventas b on b.codigo_comprobante=c.codigo_comprobante inner join compras cm on cm.codigo_comprobante=c.codigo_comprobante inner join proveedor pv on pv.pv_codigo=cm.pv_codigo inner join personal p on p.pe_codigo=c.pe_codigo where p.pe_codigo='" + Me.LBLCOD_PRODUCTO.Text + "'order by c_fecha "
    ...
    Sub LLENARLISTA1()
        Try
            ...
            cn.Open()
            cm.Connection = cn
            cm.CommandText = "select cm.c_codigo, codigo_boleta,convert(varchar(10),c_fecha,103)as fecha,total,pe_nombre+' '+pe_apellido,pv_descripcion from comprobantes c inner join boletas_de_ventas b on b.codigo_comprobante=c.codigo_comprobante inner join compras cm on cm.codigo_comprobante=c.codigo_comprobante inner join proveedor pv on pv.pv_codigo=cm.pv_codigo inner join personal p on p.pe_codigo=c.pe_codigo where pv.pv_codigo='" + Me.LBL_COD_CLIENTE.Text + "'order by c_fecha "
            cm.CommandType = CommandType.Text
            datar = cm.ExecuteReader
            Dim I As Integer = 0
            If datar.HasRows Then
    ...
    Sub LLENARLISTA3()
        Try
            ...
            cn.Open()
            cm.Connection = cn
            cm.CommandText = "select cm.c_codigo, codigo_boleta,convert(varchar(10),c_fecha,103)as fecha,total,pe_nombre+' '+pe_apellido,pv_descripcion from comprobantes c inner join boletas_de_ventas b on b.codigo_comprobante=c.codigo_comprobante inner join compras cm on cm.codigo_comprobante=c.codigo_comprobante inner join proveedor pv on pv.pv_codigo=cm.pv_codigo inner join personal p on p.pe_codigo=c.pe_codigo where c_fecha>='" + Me.DateTimePicker1.Value.Date() + "'and c_fecha<='" + Me.DateTimePicker2.Value.Date + "'order by c_fecha"
            cm.CommandType = CommandType.Text
            datar = cm.ExecuteReader
            Dim I As Integer = 0
            If datar.HasRows Then
